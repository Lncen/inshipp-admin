  ...
  configVxeTable: (vxeUI) => {
    ...
    vxeUI.renderer.add('CellTag', {
      renderTableDefault({ options, props }, { column, row }) {
        ...
        // Element Plus Tag 组件使用 type 而不是 color
        if (tagProps.color) {
          tagProps.type = tagProps.color;
    ...
    vxeUI.renderer.add('CellOperation', {
      renderTableDefault({ attrs, options, props }, { column, row }) {
        ...
        function renderBtn(opt: Recordable<any>, listen = true) {
          // 移除 Element Plus 不支持的属性
          const btnProps = { ...opt };
          if (btnProps.danger !== undefined) {
            btnProps.type = btnProps.danger
              ? 'danger'
              : btnProps.type || 'primary';
            delete btnProps.danger;
          }
          // 从属性中移除 text，因为 ElButton 的 text 属性是布尔值，不是按钮文本内容
          const { text, ...restBtnProps } = btnProps;
        ...
        function renderConfirm(opt: Recordable<any>) {
          // 处理 danger 类型按钮
          const confirmProps = { ...opt };
          if (confirmProps.danger !== undefined) {
            confirmProps.type = confirmProps.danger
              ? 'danger'
              : confirmProps.type || 'primary';
            delete confirmProps.danger;
          }
